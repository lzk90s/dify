...
def upgrade():
    ...
    with op.batch_alter_table('embeddings', schema=None) as batch_op:
        batch_op.add_column(sa.Column('model_name', sa.String(length=40),
                                      server_default=sa.text("'text-embedding-ada-002'"),
                                      nullable=False, comment='模型名'))
        batch_op.create_unique_constraint('unq_embedding_hash', ['model_name', 'hash'])
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('embeddings', schema=None) as batch_op:
        batch_op.drop_constraint('unq_embedding_hash', type_='unique')
