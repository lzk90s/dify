...
"""
    ...
    def from_texts(
        ...
        ctx = cls(config, **kwargs)
        ctx.add_texts(texts, embeddings, ids=text_ids, batch_size=batch_size, metadatas=metadatas)
        return ctx
    ...
    def similarity_search(
            self, query_vector: list[float], k: int = 4, where_str: Optional[str] = None, **kwargs: Any
    ) -> list[Document]:
        ...
        return self.similarity_search_by_vector(
            query_vector, k, where_str, **kwargs
        )
    ...
    def similarity_search_by_vector(
        ...
        """
        q_str = self._build_query_sql(query_vector, k, where_str)
        try:
            return [
                Document(
